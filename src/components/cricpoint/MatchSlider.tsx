...
export default function MatchSlider({ matches, onSelectMatch }: { matches: MatchBasic[]; onSelectMatch: (m: MatchBasic) => void }) {
  const { pinMatch, pinnedMatchId } = useCricPointStore();
          ...
          return (
            ...
            >
              <div className="w-full bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
                ...
                {/* Bottom Action Buttons: Points Table | Country | Pin */}
                <div className="border-t border-gray-100 dark:border-gray-700 px-3 py-2.5">
                  <div className="flex items-center gap-2">
                    ...
                    {/* Pin Button */}
                    <button
                      onClick={(e) => { e.stopPropagation(); pinMatch(isPinned ? null : match.id); }}
                    ...
                    >
                      <MapPin className={`w-3.5 h-3.5 ${isPinned ? 'text-green-600' : 'text-green-500'}`} />
                      <span className={`text-[11px] font-semibold ${isPinned ? 'text-green-700 dark:text-green-300' : 'text-green-700 dark:text-green-400'}`}>
                        {isPinned ? 'Pinned' : 'Pin'}
                      </span>
                    </button>
                  </div>
